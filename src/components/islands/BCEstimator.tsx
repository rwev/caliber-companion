    ...
    <div class="space-y-6">
      ...
      <div class="grid gap-4 sm:grid-cols-3">
          ...
          {customDia ? (
            <div class="flex gap-2">
              ...
              <button
                ...
                class="border border-surface-border px-2 py-2 font-mono text-xs text-text-muted hover:text-accent transition-colors"
        ...
        <div>
          ...
          <p class="mt-1 font-mono text-xs text-text-muted">{ff.desc}</p>
        </div>
      </div>
      ...
      <div class="border border-accent/30 bg-accent/5 p-4">
        <div class="grid grid-cols-3 gap-4 text-center">
          <div>
            <div class="font-mono text-xs tracking-wider uppercase text-text-muted">Sectional Density</div>
            <div class="mt-1 font-mono text-2xl font-bold text-text-primary">{results.sd}</div>
          </div>
          <div>
            <div class="font-mono text-xs tracking-wider uppercase text-text-muted">Est. BC (G1)</div>
            <div class="mt-1 font-mono text-2xl font-bold text-accent">{results.bcG1}</div>
          </div>
          <div>
            <div class="font-mono text-xs tracking-wider uppercase text-text-muted">Est. BC (G7)</div>
      ...
      <div class="border border-surface-border">
        <div class="border-b border-surface-border bg-surface-overlay px-4 py-2.5">
          <h3 class="font-display text-base tracking-[0.2em] uppercase text-text-muted">Form Factor Reference</h3>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-left font-mono">
            <thead>
              <tr class="border-b border-surface-border bg-surface-overlay text-text-muted">
                <th scope="col" class="px-3 py-2 font-medium tracking-wider uppercase text-xs">Shape</th>
                <th scope="col" class="px-3 py-2 font-medium tracking-wider uppercase text-xs text-right">G1 Factor</th>
                <th scope="col" class="px-3 py-2 font-medium tracking-wider uppercase text-xs text-right">G7 Factor</th>
                <th scope="col" class="px-3 py-2 font-medium tracking-wider uppercase text-xs">Examples</th>
              </tr>
            </thead>
            <tbody>
              {FORM_FACTORS.map((f, i) => (
      ...
      <div class="border border-surface-border-subtle bg-surface-overlay p-3">
        <p class="font-mono text-xs text-text-muted leading-relaxed">
          <span class="font-medium uppercase tracking-wider">Note:</span> BC estimates use the form factor method:
